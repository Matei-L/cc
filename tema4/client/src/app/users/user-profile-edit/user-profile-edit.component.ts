...
import {User} from 'src/app/utils/auth/User';
import {AzureSpeechToTextService} from './azure.speech-to-text.service';
...
export class UserProfileEditComponent implements OnInit, OnDestroy {
  ...
  constructor(private audioRecordingService: AudioRecordingService, private sanitizer: DomSanitizer,
              ...
              private authService: AuthService, private router: Router, private speechToTextService: AzureSpeechToTextService) {
  ...
  private getTranscriptForAudioBlob(audioBlob: Blob) {
    ...
    this.speechToTextService.fromBlob(audioBlob).subscribe(
      res => {
        if (res !== null) {
          this.doneSnackBar();
